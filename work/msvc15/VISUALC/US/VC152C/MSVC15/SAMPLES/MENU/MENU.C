...

{
    ...
    /* load the Bitmaps and shrink them for the menu */
    hLoadedBitmap = LoadBitmap (hInst, "checkon");
    if (hLoadedBitmap == NULL){
    	MessageBox(hwnd, "Unable to load bitmap checkon",
    				"Error", MB_ICONEXCLAMATION | MB_OK);
    	DestroyWindow(hwnd);
    }

    hbmCheckOn	= ShrinkBitmap (hwnd, hLoadedBitmap);
    DeleteObject(hLoadedBitmap);

    hLoadedBitmap = LoadBitmap (hInst, "checkoff");
    if (hLoadedBitmap == NULL){
    	MessageBox(hwnd, "Unable to load bitmap checkoff",
    				"Error", MB_ICONEXCLAMATION | MB_OK);
    	DestroyWindow(hwnd);
    }

    hbmCheckOff = ShrinkBitmap (hwnd, hLoadedBitmap);
    DeleteObject(hLoadedBitmap);
 ...
 *									    *
 ****************************************************************************/
long FAR PASCAL __export MenuWndProc (HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
