...
int nCmdShow;				     /* show-window type (open/icon) */
{
    ...
    {
        MessageFilterHook=SetWindowsHookEx(WH_MSGFILTER, (FARPROC)HelpMessageFilterHook, hInstance, GetWindowTask(hWndGlobal));
    }
...
/****************************************************************************
	...
	To process the IDM_ABOUT message, call Dialog
	box which will create the box according to the information in your
	HelpHook.rc file and turn control over to the About() function.

****************************************************************************/

long __export CALLBACK MainWndProc(hWnd, message, wParam, lParam)
    ...
    switch (message)
    {
		case WM_COMMAND:	   /* message: command from application menu */
        {
            switch (wParam)
            {
                ...
                case IDM_DLG9:
                {
                    DialogBox(hInst, MAKEINTRESOURCE(wParam), hWnd, SampleDialog);
                    break;
                }
...
DWORD __export CALLBACK HelpMessageFilterHook(nCode, wParam, lpMsg)
int nCode;
WORD wParam;
LPMSG lpMsg;
{
...
BOOL __export CALLBACK SampleDialog(hDlg, message, wParam, lParam)
HWND hDlg;                                /* window handle of the dialog box */
unsigned message;                         /* type of message                 */
WORD wParam;                              /* message-specific information    */
LONG lParam;
    ...
    switch (message)
    {
	    ...
	    case WM_COMMAND:              /* message: received a command */
            switch (wParam)
            {
                ...
                case IDM_DLG9:
                {
                    DialogBox(hInst, MAKEINTRESOURCE(wParam), hDlg, SampleDialog);
                    break;
                }
