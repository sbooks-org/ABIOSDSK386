...
BOOL App_InitializeHook(APP* papp)
{
    ...
    hhook = SetWindowsHookEx(WH_MSGFILTER, (FARPROC)App_MsgFilterHook, g_app.hinst, GetWindowTask(g_app.hwndMain));
    if (hhook == NULL)
        return FALSE;
...
//
LRESULT __export CALLBACK App_MsgFilterHook(int code, WPARAM wParam, LPARAM lParam)
{
