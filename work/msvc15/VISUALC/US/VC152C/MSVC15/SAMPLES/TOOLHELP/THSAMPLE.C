...
LONG __export CALLBACK WndProc			(HWND hWnd, UINT wMessage, WPARAM wParam, LPARAM lParam);
LONG __export CALLBACK MemWndProc		(HWND hWnd, UINT wMessage, WPARAM wParam, LPARAM lParam);
void PASCAL DumpMem						(HWND hwnd, HDC hDC, SHORT nScrollPos, SHORT nPos);
LONG WalkGlobalHeap						(HWND hwnd);
LONG WalkFreeList						(HWND hwnd);
LONG WalkLRUList						(HWND hwnd);
LONG WalkLocalHeap						(HWND hwnd, HANDLE hBlock);
LONG WalkModuleList						(HWND hwnd);
LONG WalkTaskList						(HWND hwnd);
LONG WalkClassList						(HWND hwnd);
LONG DoStackTrace						(HWND hwnd);
LONG DoHeapInfo							(HWND hwnd);
LONG DoMemManInfo						(HWND hwnd);
LONG DoGlobalEntryModuleTest			(HWND hwnd);
LONG ReadMemoryTest						(HWND hwnd, HANDLE hBlock);
LONG TimerCountTest						(HWND hwnd);
BOOL __export CALLBACK MyNotifyHandler	(WORD wID, DWORD dwData);

WORD __export _cdecl MyCFaultHandler(WORD wES, WORD wDS, WORD wDI, WORD wSI, WORD wBP, WORD wSP, WORD wBX,
		WORD wDX, WORD wCX, WORD wOldAX, WORD wOldBP, WORD wRetIP, WORD wRetCS, WORD wRealAX,
        WORD wNumber, WORD wHandle, WORD wIP, WORD wCS, WORD wFlags);

BOOL __export CALLBACK FaultDialogProc	(HWND hDlg, WORD wMessage, WORD wParam, DWORD dwParam);
...
char bHasData[] = { 0, 1, 1, 1, 0, 1, 0, 0, 0, 0, 0, 0 };
char *szLocalFlags[] = { "", "Fixed", "Free", "", "Moveable" };
char *szGDI[] =
{
...
int PASCAL WinMain(HANDLE hInstance, HANDLE hPrevInstance, LPSTR lpszCmdLine, int nCmdShow)
{
    ...
    if (!InterruptRegister(NULL, (FARPROC)MyCFaultHandler))
    {
    ...
    if (!NotifyRegister(
	NULL, (LPFNNOTIFYCALLBACK)MyNotifyHandler, (WORD)NF_TASKSWITCH | NF_RIP))
    {
